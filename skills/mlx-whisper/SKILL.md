...
首次使用时（`config.json` 不存在），展示以下模型选项表，询问用户选择：

| # | 模型 | 大小 | 速度 | 准确率 | 适合场景 |
|---|------|------|------|--------|---------|
| 1 | base | ~137MB | 很快 | 一般 | 快速试用，先看效果 |
| 2 | small | ~459MB | 快 | 良好 | 快速预览，准确率要求不高 |
| 3 | large-v3-turbo | ~1.5GB | 较快 | 优秀 | **推荐**，速度和质量平衡 |
| 4 | large-v3 | ~2.9GB | 慢 | 最佳 | 最终版，最高准确率 |
| 5 | large-v2 | ~2.9GB | 慢 | 很好 | 备用，某些场景更稳 |

- 默认推荐 `large-v3-turbo`（编号 3）
...
| `--model, -m` | 模型（base/small/large-v3-turbo/large-v3/large-v2） | 读取 config.json |
